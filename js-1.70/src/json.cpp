...
typedef struct _StringifyClosure
{ 
    JSContext *cx;
    jsval *s;
} StringifyClosure;
...
#if JS_HAS_TOSOURCE
static JSBool
json_toSource(JSContext *cx, JSObject *obj, uintN argc, jsval *vp, jsval* rval)
{
    *rval = ATOM_KEY(CLASS_ATOM(cx, JSON));
    return JS_TRUE;
}
#endif
...
#if JS_HAS_TOSOURCE
    {js_toSource_str,  json_toSource,      0, 0, 0},
#endif
    {"parse",          js_json_parse,      2, 0, 0},
    {"stringify",      js_json_stringify,  3, 0, 0},
    {0,0,0,0,0}
};
